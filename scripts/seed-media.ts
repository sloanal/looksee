...
const mediaList: MediaItem[] = [
  ...
  { title: 'Suspiria', type: 'MOVIE', category: 'Thriller/Horror' },
  ...
  { title: 'Heal', type: 'DOCUMENTARY', category: 'Other' },
  { title: 'Palm Springs', type: 'MOVIE', category: 'Other' },
]
...
async function main() {
  ...
  if (!finalUserId) {
    ...
    console.error('Example: USER_EMAIL=your@email.com npm run seed:media')
    console.error('Or: USER_ID=xxx ROOM_ID=xxx npm run seed:media')
    process.exit(1)
  }
  ...
  console.log(`\nSeeding media into room: "${membership.room.name}"`)
  ...
  for (let i = 0; i < mediaList.length; i++) {
    ...
    console.log(`[${i + 1}/${mediaList.length}] Processing: ${item.title}`)

    try {
      // Check if item already exists in room (case-insensitive)
